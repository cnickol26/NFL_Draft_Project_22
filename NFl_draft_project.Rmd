...
```{r}
## Only going to use players after 2009

draft_wr<-draft %>% filter(Position=="WR") %>% filter(Year > 2009) 
...
```{r}
draft_stats<-read.csv("draft_wr_stats.csv")
...
```{r}
draft_con<-draft_wr_stats %>% left_join(conferencesf,by=c("School"="TEAM"))
```
...
```{r}
draft_model_wr<-draft_con %>% select(-c(Rush.Att,Rush.TD,Rush.Yard))
draft_model_wr <- draft_model_wr %>% left_join(ras,by=c('Name' = 'Name',
                                                        'School' = 'College', 
                                                        'Year' = 'Year'))
...
```{r}
cor(wr_scale$Round, wr_scale$avg_stats)
```

```{r}
result3 <- lm(Overall ~ avg_stats + P_five, data = wr_scale)
summary(result3)
```

```{r}
wr_scale <- wr_scale %>% mutate(P_five = as.factor(P_five))
wr_scale$predict<-round(predict(result,newdata = wr_scale))
test_mse_ln<-mean((wr_scale$Overall-wr_scale$predict)^2)
test_mse_ln
```

```{r}
train<-wr_scale %>% filter(Year < 2020) %>% mutate(P_five=as.factor(P_five))
test<-wr_scale %>% filter(Year > 2019) %>% mutate(P_five=as.factor(P_five))
```

```{r}
result<-lm(Overall~avg_stats+P_five,data=train)
test$predict<-round(predict(result,newdata = test))
test_mse_ln<-mean((test$Overall-test$predict)^2)
test_mse_ln
...
```{r}
result2<-lm(Round~avg_stats+P_five,data=train)
test$predict<-round(predict(result2,newdata = test))
test_mse_ln<-mean((test$Round-test$predict)^2)
test_mse_ln
